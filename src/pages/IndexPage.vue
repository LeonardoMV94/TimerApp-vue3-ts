  ...
  <q-page padding>
    <div id="temporizador" class="row justify-center fixed-center">
      <div class="q-mb-md text-center">
        <div v-if="!isFinished">
          <q-banner inline-actions :class="`bg-${isFinished ? 'negative' : 'secondary'} text-white ${minutos == 0 && segundos <= 5 && milisegundos >= 1 && !isFinished ? 'pulsar' : ''}`">
            <span id="time" :class="`text-bold `" :style="`font-size: ${sizeTexto}rem;`">{{ formatTime(minutos) }}:{{ formatTime(segundos) }}:{{
              formatTimeMilisegundos(milisegundos) }}</span>
            <ul id="lista">
              <li v-if="overTime != 0" class="text-white q-py-lg" style="font-size: 4rem;"> + {{ overTime }} segundos
              </li>
            </ul>
          </q-banner>
        </div>
        <div v-else>
          <span class="text-bold text-white " :style="`font-size: ${sizeTexto}rem;`">{{ textoFin }}</span>
        </div>
          ...
          <q-card>
            <q-card-section :class="colorFondo2">
              ...
              <div class="row justify-center q-gutter-md q-mb-md">
                ...
                <q-btn v-for="(seg, index) of listadoSegundosDefault" :label="`+${seg} segundos`" color="blue"
                  @click="timerStore.addTime(seg, 'segundos')" />
              ...
              <div v-if="isDisableCustom" class="row justify-center q-gutter-md">
                <q-input color="blue" bg-color="blue" class="text-white" label-color="white" filled
                  v-model.number="tiempoAddBtn" label="segundos" />
    ...
    <q-dialog v-model="dialog" backdrop-filter="blur(4px)">
      <q-card class="q-pa-md">
        ...
        <q-card-section>
          ...
          <q-input v-model="timerStore.titulo" label="Titulo personalizado running timer"></q-input>
          <q-input v-model="textoFin" label="Titulo personalizado stop timer"></q-input>
          <q-select v-model="sizeTexto" :options="[4,6,8,10,12,14,16,18,20]" label="Tamaño de texto de fin timer" />
          <q-select v-model="cantidadNumeros" :options="[1, 2, 3, 4, 5]" label="Cantidad de botones" />
          <q-input v-for="(seg, index) in cantidadNumeros" :key="index" label="Segundos"
            v-model.number="listadoSegundosDefault[index]"></q-input>
        </q-card-section>
...
const textoFin = ref<string>('¡TIEMPO!')
const sizeTexto = ref<number>(8)
